    ...
    <div class="flex-1 flex items-center justify-center bg-color3 p-8">
      <form class="w-full bg-white p-4 rounded shadow-md" @submit.prevent="sendEmail">
        ...
        <div class="mb-4">
          ...
          <input class="shadow appearance-none border rounded w-full py-2 px-3 text-black leading-tight focus:outline-none focus:shadow-outline" id="name" type="text" placeholder="Seu nome" v-model="form.name" required>
        </div>
        <div class="mb-4">
          <label class="block text-black text-md font-bold mb-2" for="email">
            E-mail*
          </label>
          <input class="shadow appearance-none border rounded w-full py-2 px-3 text-black leading-tight focus:outline-none focus:shadow-outline" id="email" type="email" placeholder="Seu e-mail" v-model="form.email" required>
        </div>
        <div class="mb-4">
          <label class="block text-black text-md font-bold mb-2" for="title">
            Título*
          </label>
          <input class="shadow appearance-none border rounded w-full py-2 px-3 text-black leading-tight focus:outline-none focus:shadow-outline" id="title" type="text" placeholder="Título" v-model="form.title" required>
        </div>
        <div class="mb-6">
...
export default defineComponent({
  ...
  setup() {
    ...
    const sendEmail = () => {
      ...
      emailjs.send('sociop', 'template_y0zhpnf', templateParams, 'YsaVbURiIy4wRrTsR')
        .then((response) => {
